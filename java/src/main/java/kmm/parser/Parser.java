...
public record Parser<A>(Function<String, Optional<Pair<A, String>>> p) {
    ...
    public <B> Parser<B> map(Function<? super A, B> f) {
        return new Parser<>(inp -> parse(inp).map(
                pair -> Pair.of(f.apply(pair.first()), pair.second())));
    ...
    public <B> Parser<B> flatMap(Function<? super A, Parser<B>> f) {
        return new Parser<>(inp -> parse(inp).flatMap(
                pair -> f.apply(pair.first()).parse(pair.second())));
    }
    ...
    public final static Parser<Character> anyChar = new Parser<>(
        inp -> !inp.isEmpty() ?

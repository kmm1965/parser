...
public class Calculator {
    ...
    private Parser<Double> factor0(){
        return expr_in_brackets()
            .orElse(Parser.apply_u(func, this::expr_in_brackets))
            .orElse(_const)
            .orElse(Parser.natural);
    }

    private Parser<Double> factor(){
        return factor0().chainr1(pow);
    }

    private Parser<Double> term(){
        return factor().chainl1(mul.orElse(div));
    }

    public Parser<Double> expr(){
        return term().chainl1(add.orElse(sub));
    }

    private static Parser<BinaryOperator<Double>> op2(char c, BinaryOperator<Double> f){
        return  Parser.symbol(c).flatMap(c2 -> Parser.pure(f));
    }
    private final Parser<BinaryOperator<Double>> add = op2('+', Double::sum);
    private final Parser<BinaryOperator<Double>> sub = op2('-', (x, y) -> x - y);
    private final Parser<BinaryOperator<Double>> mul = op2('*', (x, y) -> x * y);
    private final Parser<BinaryOperator<Double>> div = op2('/', (x, y) -> x / y);
    private final Parser<BinaryOperator<Double>> pow = op2('^', Math::pow);
    ...
    private static <A> Parser<A> def_object(String name, A value){
        return Parser._name(name).flatMap(s -> Parser.pure(value));
    }

    private final Parser<UnaryOperator<Double>> [] functions = array(
    ...
    private final Parser<UnaryOperator<Double>> func = fold(functions).token();
    ...
    private final Parser<Double> [] constants = array(
        def_object("E",        Math.E),
    ...
    private final Parser<Double> _const = fold(constants).token();
}
